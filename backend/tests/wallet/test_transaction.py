...
def test_transaction_update():
    ...
    to_first_amount_again = 25
    transaction.update(sender_wallet, first_recipient, to_first_amount_again)

    assert transaction.output[first_recipient] == first_amount + to_first_amount_again
    assert transaction.output[sender_wallet.address] == sender_wallet.balance - first_amount - next_amount - to_first_amount_again
    assert Wallet.verify(
        transaction.input['public_key'],
        transaction.output,
        transaction.input['signature']
